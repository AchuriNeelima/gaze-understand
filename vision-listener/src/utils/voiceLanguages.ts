...
export function matchCommand(text: string): VoiceCommand | null {
  ...
  // Fallback: if no strict match, map common capture verbs directly to `capture`.
  // This helps when ASR outputs slightly different phrasing (e.g. "capture image", "take photo").
  const captureFallbackVerbs = ['capture', 'snap', 'take', 'shoot', 'click', 'grab', 'get'];
  if (captureFallbackVerbs.some(v => norm.includes(v))) {
    console.log('[VoiceCmd] Fallback MATCHED: capture');
    return 'capture';
  }
