    ...
    const timer = setTimeout(() => {
      switch (lastCommand) {
        ...
        case 'reset':
          ...
          break;
        case 'submit':
          toast({ title: '🎤 Voice Command', description: `"${recognizedText}" → Please upload an image first.` });
          break;
      }
      clearRecognizedText();
    }, 600);
  ...
  return (
    <div className="relative">
      <Button
        ...
        size="lg"
        onClick={handleClick}
        aria-label={isListening ? 'Stop voice recognition' : 'Start voice command'}
        className={cn(
      ...
      {recognizedText && lastCommand === 'unknown' && (
        <div className="absolute top-full mt-2 right-0 bg-card border border-border rounded-lg px-3 py-2 text-sm text-muted-foreground shadow-lg whitespace-nowrap z-50 animate-fade-in">
